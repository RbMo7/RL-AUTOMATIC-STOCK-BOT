...
# --- 3. Backtesting ---
def backtest_and_visualize(model, test_env_df, test_df_viz, initial_balance, transaction_fee_percent=0.001):
    aligned_test_df_viz = test_df_viz.loc[test_env_df.index]
    test_env = StockTradingEnv(test_env_df, initial_balance=initial_balance, transaction_fee_percent=transaction_fee_percent)
    ...
    ax2.plot(signal_dates, sell_signals, 'v', markersize=1, color='red', label='Sell Signal', alpha=0.8)
...
st.title("🧠 Intelligent RL Stock Trading Agent")
st.markdown("This agent learns a unique strategy for each stock, balancing risk and reward.")
with st.sidebar:
    ...
    options = ["Train a new specialist agent"]; default_index = 0
    ...
    with col1: train_start_date = st.date_input("Training Start", datetime.date(2020, 1, 1)); test_start_date = st.date_input("Testing Start", datetime.date(2023, 1, 1))
    ...
    total_timesteps = st.number_input("Training Timesteps", min_value=100000, value=250000, step=50000)
    transaction_fee = st.slider("Transaction Fee (%)", min_value=0.0, value=0.1, step=0.05, format="%.2f%%", help="Increase to make the agent less sensitive to small price changes and reduce over-trading.")
...
if st.sidebar.button("🚀 Run Analysis", type="primary"):
    ...
    if train_start_date >= train_end_date or test_start_date >= test_end_date or train_end_date >= test_start_date:
        ...
        try:
            fee_decimal = transaction_fee / 100.0
            ...
            if train_df_processed is None or len(train_df_processed) < 50: st.error(f"Not enough training data for {ticker}. Please select a wider date range.")
            elif test_df_processed is None or len(test_df_processed) < 50: st.error(f"Not enough testing data for {ticker}. Please select a wider date range.")
            else:
                st.success("Data processing complete!")
                train_env = DummyVecEnv([lambda: StockTradingEnv(train_df_processed, transaction_fee_percent=fee_decimal)])
                model = None
                ...
                if model is None:
                    ...
                    with col2: metrics_placeholder = st.empty()
                    streamlit_callback = StreamlitCallback(total_timesteps, progress_bar, chart_placeholder, metrics_placeholder)
                    model = PPO('MlpPolicy', train_env, verbose=0, gamma=0.8, tensorboard_log=log_dir)
                ...
                with st.spinner("Running backtest and generating results..."):
                    metrics, fig, trades_df = backtest_and_visualize(model, test_df_processed, test_df_viz, initial_balance, transaction_fee_percent=fee_decimal)
